...
import os
from agents.utils import OnPolicyBuffer, MultiAgentOnPolicyBuffer, Scheduler,PPOBuffer
from agents.policies import (LstmPolicy, FPPolicy, ConsensusPolicy, NCMultiAgentPolicy,
                             IC3MultiAgentPolicy, DIALMultiAgentPolicy,IC3GateMultiAgentPolicy,MAPPOPolicy)
import logging
import numpy as np
...
class IA2C:
    ...
    def _init_algo(self, n_s_ls, n_a_ls, neighbor_mask, distance_mask, coop_gamma,
        ...
        self.update = model_config.getboolean('update', fallback=False)
        # init tf
...
class MA2C_IC3NET(MA2C_NC):

    def __init__(self, n_s_ls, n_a_ls, neighbor_mask, distance_mask, coop_gamma,
                 total_step, model_config, seed=0):
        self.name = 'ma2c_ic3net'                      
        self._init_algo(n_s_ls, n_a_ls, neighbor_mask,
                        distance_mask, coop_gamma,
                        total_step, seed, model_config)

    def _init_policy(self):

        if self.identical_agent:
            return IC3GateMultiAgentPolicy(self.n_s, self.n_a, self.n_agent,
                                       self.n_step, self.neighbor_mask,
                                       n_fc=self.n_fc, n_h=self.n_lstm,update=self.update)
            ...
            return IC3GateMultiAgentPolicy(self.n_s, self.n_a, self.n_agent,
                                       ...
                                       n_s_ls=self.n_s_ls,
                                       n_a_ls=self.n_a_ls,
                                       identical=False,update=self.update)
    def backward(self, Rends, dt, summary_writer=None, global_step=None):
        """
        把整个动作矩阵 `acts` 当作 nactions 传入，
        与 IC3GateMultiAgentPolicy.action_bw = [N_agent, n_step] 对齐。
        """
        cur_lr = self.lr_scheduler.get(self.n_step)
        obs, _ps, acts, dones, Rs, Advs = \
            self.trans_buffer.sample_transition(Rends, dt)

        nactions = acts.astype(np.int32)

        self.policy.backward(self.sess, obs, nactions, acts, dones,
                             Rs, Advs, cur_lr,
