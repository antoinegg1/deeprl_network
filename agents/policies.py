...
class IC3GateMultiAgentPolicy(Policy):
    """IC3Net policy with learnable gating.
    If identical=False, n_s_ls / n_a_ls must be provided and obs/action
    tensors are expected to be **padded** to max dims, mirroring
    IC3MultiAgentPolicy behaviour.
    """

    def __init__(self, n_s, n_a, n_agent, n_step, neighbor_mask,
                 n_fc=64, n_h=64, lambda_gate=1e-3,
                 n_s_ls=None, n_a_ls=None, identical=True,update=False):

        super().__init__(n_a, n_s, n_step, 'ic3gate', None, identical)
        if not identical:
        ...
        self.update=update
        if self.update:
            self.gate=lstm_ic3_gate_update
        else:
            self.gate=lstm_ic3_gate
        self._init_policy(n_agent, neighbor_mask, n_h)


    # ------------------------------------------------------------------
    # Overridden Policy helpers ----------------------------------------
    # ------------------------------------------------------------------
    def _init_policy(self, n_agent, neighbor_mask, n_h):
        # placeholders (same shape for both branches; obs must be padded if hetero)
        self.ob_fw     = tf.placeholder(tf.float32, [n_agent, 1, self.n_s])
        self.action_fw = tf.placeholder(tf.int32,  [n_agent, 1])
        self.done_fw   = tf.placeholder(tf.float32, [1])
        self.ob_bw     = tf.placeholder(tf.float32, [n_agent, self.n_step, self.n_s])
        self.action_bw = tf.placeholder(tf.int32,  [n_agent, self.n_step])
        self.done_bw   = tf.placeholder(tf.float32, [self.n_step])
        self.states    = tf.placeholder(tf.float32, [n_agent, n_h * 2])

        with tf.variable_scope(self.name):
            self.pi_fw, self.v_fw, self.new_states, self.g_fw = self._build_net('forward')
        with tf.variable_scope(self.name, reuse=True):
            self.pi, self.v, _, self.g_bw = self._build_net('backward')
        self._reset()

    def _build_net(self, in_type):
        if in_type == 'forward':
            ob, action, done = self.ob_fw, self.action_fw, self.done_fw
        else:
            ob, action, done = self.ob_bw, self.action_bw, self.done_bw

        # ------ IC3Net core ------
        scope_root = 'lstm_ic3_gate_update' if self.update else 'lstm_ic3_gate'
        h, new_states, g_seq = self.gate(ob, done, self.neighbor_mask, self.states, scope_root)
        pi_ls, v_ls = [], []
        for i in range(self.n_agent):
            h_i = h[i]
            if self.identical:
                pi = self._build_actor_head(h_i, agent_name=str(i))
                pi_ls.append(tf.expand_dims(pi, axis=0))
                n_n = int(np.sum(self.neighbor_mask[i]))
            else:
                pi = self._build_actor_head(h_i, n_a=self.n_a_ls[i], agent_name=str(i))
                pi_ls.append(tf.squeeze(pi))
                self.na_dim_ls = [self.n_a_ls[j] for j in np.where(self.neighbor_mask[i]==1)[0]]
                n_n = len(self.na_dim_ls)
            if n_n:
                naction_i = tf.transpose(tf.boolean_mask(action, self.neighbor_mask[i]))
            else:
                naction_i = None
            v = self._build_critic_head(h_i, naction_i, n_n=n_n, agent_name=str(i))
            v_ls.append(tf.expand_dims(v, axis=0))
        if self.identical:
            pi_cat = tf.squeeze(tf.concat(pi_ls, axis=0))
        else:
            pi_cat = pi_ls  # hetero: keep list of tensors
        v_cat = tf.squeeze(tf.concat(v_ls, axis=0))
        return pi_cat, v_cat, new_states, g_seq

    # ------------------------------------------------------------------
    # Loss (handles identical / hetero)
    # ------------------------------------------------------------------
    def prepare_loss(self, v_coef, e_coef, max_grad_norm, alpha, epsilon):
        if self.identical:
            super().prepare_loss(v_coef, e_coef, max_grad_norm, alpha, epsilon)
        else:
            self.A = tf.placeholder(tf.int32, [self.n_agent, self.n_step])
            self.ADV = tf.placeholder(tf.float32, [self.n_agent, self.n_step])
            self.R   = tf.placeholder(tf.float32, [self.n_agent, self.n_step])
            entropy_list, probpi_list = [], []
            for i in range(self.n_agent):
                pi_i = self.pi[i]
                action_i = tf.slice(self.action_bw, [i,0], [1,self.n_step])
                A_sparse_i = tf.one_hot(action_i, self.n_a_ls[i])
                log_pi_i = tf.log(tf.clip_by_value(pi_i, 1e-10, 1.0))
                entropy_list.append(tf.expand_dims(-tf.reduce_sum(pi_i*log_pi_i, axis=-1),0))
                probpi_list.append(tf.expand_dims(tf.reduce_sum(log_pi_i*A_sparse_i, axis=-1),0))
            entropy = tf.concat(entropy_list, axis=0)
            prob_pi = tf.concat(probpi_list, axis=0)
            entropy_loss = -tf.reduce_sum(tf.reduce_mean(entropy,axis=-1))*e_coef
            policy_loss  = -tf.reduce_sum(tf.reduce_mean(prob_pi*self.ADV, axis=-1))
            value_loss   = tf.reduce_sum(tf.reduce_mean(tf.square(self.R-self.v),axis=-1))*0.5*v_coef
            self.loss = policy_loss + value_loss + entropy_loss
            ...
            self._train = self.optimizer.apply_gradients(list(zip(grads, wts)))
            # summaries
            summaries = [tf.summary.scalar(f'loss/{self.name}_entropy_loss', entropy_loss),
                         tf.summary.scalar(f'loss/{self.name}_policy_loss', policy_loss),
                         tf.summary.scalar(f'loss/{self.name}_value_loss', value_loss),
                         tf.summary.scalar(f'loss/{self.name}_total_loss', self.loss),
                         tf.summary.scalar(f'train/{self.name}_lr', self.lr),
                         tf.summary.scalar(f'train/{self.name}_gradnorm', self.grad_norm)]
            self.summary = tf.summary.merge(summaries)
        # ----- add gate sparsity (both cases) -----
        gate_mean = tf.reduce_mean(self.g_bw)
        self.loss += self.lambda_gate * gate_mean
        tf.summary.scalar('loss/%s_gate_mean' % self.name, gate_mean)

    # ------------------------------------------------------------------
    # backward / forward wrappers remain unchanged
    # ------------------------------------------------------------------
    def backward(self, sess, obs, nactions, acts, dones, Rs, Advs, cur_lr,
                 summary_writer=None, global_step=None):
        ins = {self.ob_bw: obs,
               self.action_bw: nactions,
               self.done_bw: dones,
               self.states: self.states_bw,
               self.A: acts,
               self.ADV: Advs,
    ...
    def forward(self, sess, ob, done, policy_=None, naction=None, out_type='p'):
        """If `policy_` is provided it is ignored (IC3Net无需fingerprint)."""
        ins = {self.ob_fw: np.expand_dims(ob, 1),
               self.done_fw: np.expand_dims(done, 0),
               self.states: self.states_fw}
        if out_type.startswith('p'):
            outs = [self.pi_fw, self.new_states]
        else:
            outs = [self.v_fw]
            ins[self.action_fw] = np.expand_dims(naction, 1)
        out_vals = sess.run(outs, ins)
        out_val  = out_vals[0]
        if out_type.startswith('p'):
            self.states_fw = out_vals[-1]
        return out_val
    
    def _reset(self):
        self.states_fw = np.zeros((self.n_agent, self.n_h*2), np.float32)
        self.states_bw = np.zeros((self.n_agent, self.n_h*2), np.float32)
